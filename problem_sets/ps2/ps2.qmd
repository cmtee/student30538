...
```{python}
# Reading in file
file_path = r'C:\Users\clari\OneDrive\Documents\Python II\ppha30538_fall2024\problem_sets\ps2\data\parking_tickets_one_percent.csv'
...
```{python}
filtered_violations = df[df['violation_description'].str.contains('NO CITY STICKER')]
...
## 1.4
```{python}
violation_fines = filtered_violations[filtered_violations['violation_code'].isin(['0964125', '0964125B', '976170'])][['issue_date', 'violation_code', 'fine_level1_amount', 'fine_level2_amount']]
print(violation_fines)

violation_fines_descending = violation_fines.sort_values(by = 'fine_level2_amount', ascending = False)
print(violation_fines_descending)

violation_fines_ascending = violation_fines.sort_values(by = 'fine_level2_amount')
...
# Replace the specified codes with the new code
...
```{python}
sticker_violations_monthly = merged_sticker_violations[merged_sticker_violations['violation_code'].isin(['012345'])][['issue_date','violation_code', 'ticket_queue']]
sticker_violations_monthly['issue_date'] = pd.to_datetime(sticker_violations_monthly['issue_date'])

# Monthly aggregation
sticker_violations_dt = sticker_violations_monthly.groupby(sticker_violations_monthly['issue_date'].dt.to_period('M').dt.to_timestamp()).size().reset_index(name="count")
```
...
# Create the chart
chart = alt.Chart(sticker_violations_dt).mark_line(point=True).encode(
...
price_increase_line = alt.Chart(pd.DataFrame({'date': ['2012-01-01']})).mark_rule(
    color='red',
    strokeWidth=2
).encode(
    x='yearmonth(date):T'
...
# Combine the charts
final_chart = chart + price_increase_line
...
2.3 
```{python}
sticker_violations_2011 = (sticker_violations_dt[sticker_violations_dt['issue_date'].dt.year == 2011])
...
```{python}
prediction = 1935 * 100 * (80)
print(f'They should have predicted an increase of  {prediction} USD in revenue')
```

*need to add some commas

2.4
What happened to repayment rates (percentage of tickets issued that had payments
made) on this type of ticket in the calendar year after the price increase went into effect?

```{python}
sticker_violations_2012 = sticker_violations_monthly[sticker_violations_monthly['issue_date'].dt.year == 2012]

issued_2012 = sticker_violations_2012['ticket_queue'].count()

print(issued_2012)
# Count paid tickets for 2012
paid_2012 = sticker_violations_2012[sticker_violations_2012["ticket_queue"] == "Paid"]['ticket_queue'].count()

# Calculate ticket payment rate
ticket_pay_rate = (paid_2012 / issued_2012) * 100

# Calculate ticket payment rate
ticket_pay_rate = (paid_2012 /issued_2012) * 100

print(ticket_pay_rate)
...
2.5
Make a plot with the repayment rates on “missing city sticker” tickets and a vertical line
